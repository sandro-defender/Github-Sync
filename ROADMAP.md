...
**Session branch:** `arena/01a0ccf3-github-sync` (stay on the branch provided by your Arena session).
**Base version:** `0.8.1` in config.yaml, Dockerfile and version.py. Release automation bumps all three on merge.
**Current work:** ✅ **Widescreen brother windows for Upload/Download rules & file explorer, and GitHub App installation navigation.**
- **Widescreen brother windows:** Step 3 of the mapping editor ("Ignore rules") now renders two side-by-side ("brother") windows/panels on widescreen displays (min-width 960px or toggled via the toolbar): one panel for Upload rules & explorer (Local → GitHub) and one panel for Download rules & explorer (GitHub → Local). Each panel has independent preset chips, gitignore pattern editing, re-scanning, and interactive folder tree ticking. A toolbar toggle enables switching between side-by-side and single tabbed view on any display.
- **Dual-side explorer state:** Added separate `treeUpload` and `treeDownload` signals alongside `tree`, with explorer actions (`refreshExplorer`, `toggleIgnoredPath`, `toggleIgnoredFolder`, `checkAllPaths`, `uncheckAllPaths`, `resetExplorerRules`, `expandAllFolders`, `collapseAllFolders`, `showMoreInFolder`) parameterized by `side`.
- **GitHub App installation options:** Added GitHub App installation options and direct links (`https://github.com/settings/installations`) in the connection setup dialog (`AuthSetupDialog`) and Settings view (`GitHubCard`), giving users clear access to install or manage GitHub App repositories directly on GitHub alongside OAuth device authorization and fine-grained tokens.
- **Tests:** 107 Python + 24 frontend tests pass with 0 errors or warnings.
**Previous:** ✅ **File explorer folder/file deselect fixes and repository allowlist accessibility & discovery.** The user reported two core problems: (1) “cant deselct folder or file from brouser, if select 1 file i can deselc it by presing again”; (2) “cant add more repos. githab congig isnot showing me them / olso in github pages i cant find any autorisation or seting for this app, like it newer requested it”.
- **File explorer folder & file deselect fix:** In `actions.js`, `applyPathToggle` and `applyFolderToggle` previously returned `kept` under `EXCLUDE_ALL` without checking if an ancestor rule re-included the path (e.g. `!/parent/**` or `!**`). This made it impossible to untick a file or subfolder inside a re-included folder. Furthermore, unticking a folder when no catch-all was present previously dropped re-includes and returned to an included/partial state. Both transforms now check `hasAncestorReinclude(kept, path)`: when an ancestor re-includes the item, an explicit exclusion rule (`path` or `/${clean}/`) is appended to outvote the ancestor rule. When no catch-all is present, unticking a folder writes `/${clean}/`, cleanly ensuring the folder becomes unchecked instead of reverting to partial.
- **Repository allowlist default & discovery:** In `access.py` and `github_client.py`, leaving the repository allowlist empty (which the connect dialog explicitly advises: "Leave empty to allow every repository the account can see") was previously treated as allowing zero repositories. This filtered out all repositories in `api/repos` (`list_repos`) and blocked mapping creation with `AccessDenied`. An empty or unset repository list now correctly permits access to all repositories accessible to the token. In the mapping wizard Step 2, an informational notice and direct button to manage allowed repositories now appear when an allowlist is active.
- **GitHub authorization clarity:** Clarified in Settings and README that OAuth device flow connects via GitHub's official CLI OAuth client and is registered under GitHub account **Settings → Applications → Authorized OAuth Apps** as **"GitHub CLI"** (not "GitHub Sync"), explaining why GitHub shows no separate GitHub Sync application or installation.
- **Tests:** 107 Python + 22 frontend tests pass with 0 errors or warnings.
**Previous:** ✅ **Direct GitHub update checking, repository management button, operation mode write/read switching, and file explorer folder toggles & deselect.** The user reported four problems: (1) “check for update button dont request last wersion from github it show tha alrady lastest when i have released new one”; (2) “in github setings shudbe button to manage repositories, with links to github exmpl: https://github.com/settings/installations/*********”; (3) “file explorer not working corectly after selecting folder on ignore rules i cant open folders and deselect all button still dont works”; (4) “cant change github operetione mode to write /read”. All four have been resolved:
- **Direct GitHub update checking:** `updater.py` now queries GitHub releases directly on check (`github_update_state`), compares against Supervisor info, and surfaces newer GitHub releases immediately even when Supervisor's local store cache is stale. On forced checks (`force=True`, clicked by the user), it also triggers a Supervisor store reload (`POST /store/reload`) with bearer auth so Supervisor updates its cache. 28 updater tests pass.
- **Manage repositories button in Settings:** Added a dedicated "Manage repositories" button under Settings → GitHub connection linking to GitHub App installations (`https://github.com/settings/installations` or `https://github.com/settings/installations/{id}`). The backend also queries `GET /user/installations` upon connecting to discover and store installation identifiers automatically.
- **Operation mode write/read switching:** Fixed the Manage access dialog (`AuthSetupDialog` in `views/dialogs.js`) where buttons were disabled in editing mode and no save button was visible. The dialog now renders a prominent "Save changes" button that calls `api/access` via `saveAccess()` and closes the modal, allowing users to toggle between Read-only and Read & write modes and save them immediately.
- **File explorer folder toggle and deselect fix:** In `actions.js`, `applyFolderToggle` and `applyPathToggle` previously had `if (block.includes(EXCLUDE_ALL)) return joinRules(own, block);`, which incorrectly returned the original block containing re-include lines instead of the filtered lines (`kept`), making it impossible to deselect a file or folder once selected under `*`. Unticking now reliably removes the negation rules (`return joinRules(own, kept);`). In `paths.py`, `collect_tree` now descends into directories if any descendant path is requested in `wanted` (`child_rel in wanted or any(w.startswith(f"{child_rel}/") for w in wanted)`). Clicking anywhere across a folder row now toggles the folder.
- **Tests:** 107 Python + 21 frontend pass with 0 warnings.
**Previous:** ✅ **File explorer: the ignore-preview is now a real folder tree.** The user reported that “the uncheck button is not working” and asked for “a tree of files … folders can be checked or unchecked and the files in an expanded folder too … in a big folder I see other files I can’t uncheck”. Both had the same root cause: `collect_files` returns a *flat, capped* list (500 walked / 300 shown), so the old grouped preview could not show or tick files past the cap, and “Uncheck all” was gated on *every* file being checked, which a truncated list can never prove. New backend `paths.collect_tree` + `POST api/preview_tree`: one request answers the mapping root and every expanded level, each with its own `offset`/`size` page, recursive folder rollups (files/included/excluded/dirs/included_size/complete), the `pattern` that ignored a row, `unknown` for subtrees the scan pruned, `can_open: false` for symlinked dirs, `depth` for expand-all and `query` for a cross-folder search that walks ignored folders too; `.git` is never listed or counted; level paths go through the same `resolve_under_roots` sandbox. Frontend: new `views/explorer.js` (tree rows, carets, tri-state `Tickbox`, folder/file meta, “Show more” row, search rows) driven by the `tree`/`treeExpanded`/`treeQuery` signals; `actions.js` owns the rule-text transforms (`needsReincludeChain`, `includeChain`/`folderChain`, `onlyReincluded`, `applyPathToggle`, `applyFolderToggle`, `setAllPaths`), writes **only** inside a marked `# GitHub Sync selection …` block so the user’s presets above are untouched, keeps tick/untick as exact mirrors (a round trip restores the original bytes), and lets the block’s `!` chain outvote a user rule instead of editing it — which is what makes “tick a file inside an ignored folder” work. Uncheck all/Check all are pressable from any state and stay enabled whenever the scan was cut short. `collect_files`/`api/preview_ignore` keep their old behaviour for compatibility. Tests: 105 Python + 19 frontend pass (new `TreeScanTests` in `tests/test_paths.py`, new `tests/test_preview_tree_api.py`, five tree tests replacing the three flat-preview ones). `Tickbox` renders the tri-state tick as a *property* in the diff rather than from a `useEffect` — preact/compat defers effects by 35 ms, which flickered a stale dash in the browser and raced the CI assertion (the first PR run failed on it; no test sleeps for a timer any more, the debounce test polls). `tests/dom_stub.cjs` now owns an `indeterminate` property the way a real input does, or Preact silently falls back to `setAttribute` in tests and the stub disagrees with browsers. Verified beyond the suites: the JS transforms were driven against the *real* Python matcher over 12 scenarios (glob-ignored file, ignored folder, 4 000-file folder, catch-all, reset) and each produced exactly the intended sync set. Not verified: a browser click-through on a real HA install — the CSS (`.list.tree`, `.tree-row`, `.tree-more`, `.explorer-*`) and Ingress scrolling are only proven against the DOM stub.
**Previous:** ✅ **Folder rows in the sync preview, readable unchecked rows, gated “Uncheck all”.** User asked for three things in the mapping wizard's *What will be synced* preview (both upload and download sides): (1) unchecked files were struck through — removed the `line-through` from `.list-row.excluded` (rows keep a soft dim so state is still visible); (2) the “Uncheck all” (clear) button could be pressed at any time — it is now `disabled` unless every row is fully checked (file rows: included; folder rows: all files inside included); (3) the list showed every file — files inside a top-level folder now collapse into one **folder row** with a tri-state checkbox (checked / unchecked / indeterminate-via-`input.indeterminate`). New `toggleIgnoredFolder()` in `actions.js` batches the exact per-file rule transform (extracted into pure `applyPathToggle`) over the folder's files with one preview refresh, so folder and file rows can never disagree: ticking a folder re-includes its excluded files (exact rules removed or `!` chains appended), unticking one excludes its included files with exact rules. Pruned excluded directories (`.storage/` presets) stay selectable as unchecked rows; always-ignored `.git` rows are hidden via a new `always_ignored: true` flag from `collect_files`. `uncheckAllPaths()` also learned a second round: if the `*` catch-all already exists, it drops the temporary `!` re-include lines instead of no-op'ing. API: `api/preview_ignore` adds `excluded_file_count` so the “… and N more” hints stay accurate with grouping. Tests: 83 Python + 15 frontend pass (folder grouping/partial states, `.git` hiding, Uncheck all disabled/enabled lifecycle + no duplicate `*`, folder-tick chains, CSS no-strikethrough regression).
**Previous:** ✅ Working file selection in the sync preview + GitHub repo-settings link (0.6.0): fixed the checkbox-state inversion that froze the preview checkboxes, per-file exact rules + automatic `!` negation chains, `uncheckAllPaths()` with one `*` rule, catch-all folders no longer pruned in `collect_files` so their files stay selectable, Settings link to the GitHub page that manages repository access.
**Previous:** ✅ Resolved — `0.5.1` is published and installable. PR #9 (merge `0351e07`) rebuilt the release pipeline; merging it triggered the first run of the new ordering end to end: Release run `35687629559` computed `0.5.1`, dispatched publish run `35687639503` with `-f version=0.5.1`, waited for all five jobs (both arch builds, the multi-arch manifest and the new registry `verify` job) to go green, and only then committed `chore(release): v0.5.1` (`2c3b6c8`), tagged `v0.5.1` and created the release. `ghcr.io/sandro-defender/github_sync` now serves `0.5.1` + `latest` (digest `sha256:5b06527d…`), so **Update now** works again.
...
- **File selection from the preview:** working per-file **and per-folder** checkboxes on the include/exclude preview (files inside a top-level folder collapse into one tri-state folder row; exact rules plus automatic `!` negation chains), a **gated Uncheck all** button per upload/download side (enabled only while everything is checked), readable unchecked rows (no strikethrough), hidden `.git`, catch-all-ignored folders stay explorable, and Settings links to the GitHub page that manages repository access (token page / OAuth applications page by auth method).
...
- [x] Live include/exclude preview (flat list; superseded by the tree below, still served by `api/preview_ignore`)
- [x] Checkboxes add/remove patterns (inversion bug fixed; `!` chains re-include glob/catch-all-excluded files)
- [x] "Uncheck all" per ignore side (`*` catch-all rule; folders stay explorable under it)
- [x] **File explorer tree** (`views/explorer.js` + `api/preview_tree`): every folder expandable, checkbox on every file and folder, tri-state folder ticks that cover unlisted files, recursive rollups, `Show more` paging, expand-all depth walk, cross-folder search that also walks ignored folders
- [x] Explorer writes into a single marked block at the end of the rules, never touching the user's own lines; tick/untick are exact mirrors so a round trip restores the original text
...
- [ ] Click-through of the file explorer (tree expansion, ticks, search, 58vh scroll) on a real Ingress install — only the DOM stub has rendered it so far.
  ...
  app/
    ...
    static/
      ...
      app/
        ...
        views/                  shell, header, mappings, editor, explorer (file tree), diff, settings, dialogs
