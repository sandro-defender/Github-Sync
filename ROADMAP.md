...
**Session branch:** `arena/01a0cb30-github-sync` (stay on the branch provided by your Arena session).
**Base version:** `0.5.1` in config.yaml, Dockerfile and version.py. Release automation bumps all three on merge.
**Current work:** ✅ **Working file selection in the sync preview + GitHub repo-settings link.** Users reported that the checkboxes in the mapping wizard's *What will be synced* preview could not be unchecked, and asked for an "Uncheck all" plus a Settings button to the GitHub page where repositories are added/removed. Root cause of the frozen checkboxes: `views/editor.js` passed `!ev.target.checked` into `toggleIgnoredPath`, inverting every click (untick → "include" no-op, tick → "exclude" no-op). Fixed the inversion; `toggleIgnoredPath` now re-includes glob/catch-all-excluded files via an anchored `!` negation chain (parents first, then the file — gitignore semantics) and removes only that chain's file line on untick. New `uncheckAllPaths()` appends one `*` rule so the user ticks just the files to sync; the wizard preview lists up to the API caps (500/300) with "… and N more" hints. Backend: `collect_files` no longer prunes folders whose last matching rule is a catch-all (`*`/`**`), so files under catch-all-ignored folders stay visible and selectable — hand-written folder rules keep pruning, `.git` always prunes, sync verdicts unchanged. Settings → GitHub connection now links to `github.com/settings/personal-access-tokens` for fine-grained token connections (add/remove repos under *Repository access*) and to `github.com/settings/applications` for device/OAuth sign-ins (with a hint that OAuth grants have no per-repo limits). Tests: 83 Python + 13 frontend pass (new: real checkbox `change`-event flows in `tests/test_frontend.cjs`, catch-all expansion in `tests/test_paths.py`); the `*` + `!chain` behaviour was also verified against a live `api/preview_ignore` endpoint.
...
- **File selection from the preview:** working per-file checkboxes on the include/exclude preview (exact rules plus automatic `!` negation chains), an **Uncheck all** button per upload/download side, catch-all-ignored folders stay explorable, and Settings links to the GitHub page that manages repository access (token page / OAuth applications page by auth method).
