...
**Base version:** `0.5.0` in config.yaml, Dockerfile and version.py. Release automation bumps all three on merge.
**Current work:** ⚠️ **`0.5.0` is advertised but uninstallable.** PR #8 pinned `image: "ghcr.io/sandro-defender/github_sync"` in `config.yaml`, the release bumped to `0.5.0`, and the dispatched publish run was rejected by GHCR: `failed to push ghcr.io/sandro-defender/aarch64-github_sync:0.5.0: denied: permission_denied: write_package` (run `35684742281`). Both build jobs died, so the manifest job never ran and `ghcr.io/sandro-defender/github_sync` only has `0.4.0` + `latest`. Users who press **Update now** get `Can't install ghcr.io/sandro-defender/github_sync:0.5.0: [404] manifest unknown` — Supervisor pulls `<image>:<version>` and does not fall back to a local build. Cause: making the three packages **public** (the documented step after `0.4.0`) switches them from "inherit access from the linked repository" to granular permissions, and GitHub overwrites the existing permissions when that changes — which revoked the `GITHUB_TOKEN` write access that published `0.4.0`. **The registry fix is manual and still outstanding**; this session made the pipeline detect and prevent the failure instead.
**Recovery (needs a browser + repo admin, in this order):** for each of `github_sync`, `aarch64-github_sync`, `amd64-github_sync` → `https://github.com/users/sandro-defender/packages/container/package/<name>` → *Package settings → Manage Actions access → Add repository → `sandro-defender/Github-Sync` → Role: Write*; then `gh run rerun 35684742281` (republishes `0.5.0` + `latest` from the bump commit) and confirm with `.github/scripts/check_published_images.sh 0.5.0`. Alternative that needs no package UI: store a classic PAT with `write:packages` as the repository secret `GHCR_TOKEN` (plus `GHCR_USERNAME` as a variable) — `publish.yml` prefers it over `GITHUB_TOKEN`.
...
1. **Unblock `0.5.0`** — re-grant Actions write access on the three GHCR packages, `gh run rerun 35684742281`, confirm with `.github/scripts/check_published_images.sh 0.5.0` (see *Recovery* above). Everything else waits on this.
2. Verify on a real HA OS/Supervised install: install/update flow (now pulling a pre-built image), device authorization, fine-grained read-only repos, auto-sync denial, dry-run previews, and the panel on mobile + light theme (checklist in `docs/store-submission.md`).
...
- [ ] **Blocking:** re-grant Actions write access on all three packages (*Package settings → Manage Actions access → Add repository → Write*), which making them public silently revoked, then republish `0.5.0` (`gh run rerun 35684742281`) and confirm with `.github/scripts/check_published_images.sh 0.5.0`. Until then `0.5.0` installs/updates fail with `[404] manifest unknown`.
